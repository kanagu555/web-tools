...
const AgeCalculator: React.FC = () => {
  ...
  }, [calculatedAge, birthDate, copyToClipboard, showSnackbar]);

  const downloadAgeDetails = useCallback(() => {
    ...
    downloadElement.innerHTML = `
      ...
      <div style="text-align: center; margin-top: 30px; opacity: 0.7;">
        <p style="font-size: 14px; margin: 0;">Generated on ${new Date().toLocaleDateString()}</p>
        <p style="font-size: 12px; margin: 5px 0 0 0;">Age Calculator - kodekit.in</p>
      </div>
    `;
    ...
    html2canvas(downloadElement, {
      ...
      allowTaint: true,
      width: 800,
      height: downloadElement.offsetHeight,
    })
      .then((canvas) => {
        try {
          ...
          showSnackbar("Age details downloaded successfully!", "success");
        } catch (error) {
      ...
      .catch((error) => {
        ...
        showSnackbar("Failed to generate download. Please try again.", "error");
        document.body.removeChild(downloadElement);
      });
  }, [calculatedAge, showSnackbar]);
