...
class SEOValidator {
    ...
    return new Promise((resolve) => {
      try {
        ...
        if (!fs.existsSync(buildDir)) {
          resolve({
            passed: false,
            message: 'Next.js build directory not found. Run "npm run build" first.'
          });
          return;
        }
    ...
    return new Promise((resolve) => {
      try {
        ...
        resolve({
          ...
          message: issues.length > 0 ? issues.join(', ') : 'All metadata configurations are present'
        });
      } catch (error) {
        resolve({
          passed: false,
    ...
    return new Promise((resolve) => {
      try {
        ...
        for (const filePath of files) {
          ...
          if (content.includes('og-image') || content.includes('twitter-image')) {
        ...
        resolve({
          passed: issues.length === 0,
          message: issues.length > 0 ? issues.join(', ') : 'Social media metadata is properly configured'
        });
      } catch (error) {
        resolve({
          passed: false,
    ...
    return new Promise((resolve) => {
      try {
        ...
        if (!fs.existsSync(robotsPath)) {
          ...
          if (!robotsContent.includes('userAgent') || !robotsContent.includes('sitemap')) {
        ...
        // Check if build generates these files
        const publicRobots = path.join(process.cwd(), 'public', 'robots.txt');
        const publicSitemap = path.join(process.cwd(), 'public', 'sitemap.xml');
        
        // Note: These files are generated during build, so we check the source files instead
        resolve({
          passed: issues.length === 0,
          message: issues.length > 0 ? issues.join(', ') : 'Search engine files are properly configured'
        });
      } catch (error) {
        resolve({
          passed: false,
  ...
  async testStructuredData() {
    return new Promise((resolve) => {
      try {
        ...
        const structuredDataPath = path.join(process.cwd(), 'lib', 'utils', 'structuredData.ts');
        ...
        // Check if pages use structured data
        const pageFiles = [
          'app/page.tsx',
          'app/tools/[toolName]/page.tsx',
          'app/category/[categoryId]/page.tsx'
        ...
        for (const pageFile of pageFiles) {
          ...
          if (fs.existsSync(pagePath)) {
            ...
            if (!pageContent.includes('StructuredData')) {
        ...
        resolve({
          passed: issues.length === 0,
          message: issues.length > 0 ? issues.join(', ') : 'Structured data is properly implemented'
        });
      } catch (error) {
        resolve({
          passed: false,
  ...
  async testCanonicalUrls() {
    return new Promise((resolve) => {
      try {
        ...
        // Check canonical URL utility
        const canonicalPath = path.join(process.cwd(), 'lib', 'utils', 'canonicalUrl.ts');
        if (!fs.existsSync(canonicalPath)) {
          issues.push('canonicalUrl.ts utility not found');
        }

        // Check if pages include canonical URLs
        const pageFiles = [
          'app/layout.tsx',
          'app/page.tsx',
          'app/tools/[toolName]/page.tsx',
        ...
        for (const pageFile of pageFiles) {
          ...
          if (fs.existsSync(pagePath)) {
            ...
            if (!pageContent.includes('canonical')) {
              issues.push(`${pageFile} missing canonical URL`);
            }
          }
        }
        ...
        resolve({
          passed: issues.length === 0,
          message: issues.length > 0 ? issues.join(', ') : 'Canonical URLs are properly configured'
        });
      } catch (error) {
        resolve({
          passed: false,
  ...
  async testUrlStructure() {
    return new Promise((resolve) => {
      try {
        ...
        const toolsDataPath = path.join(process.cwd(), 'lib', 'data', 'toolsData.ts');
        ...
        resolve({
          passed: issues.length === 0,
          message: issues.length > 0 ? issues.join(', ') : 'URL structure is SEO-friendly'
        });
      } catch (error) {
        resolve({
          passed: false,
  ...
  async testPerformanceOptimization() {
    return new Promise((resolve) => {
      try {
        ...
        const perfMonitorPath = path.join(process.cwd(), 'components', 'PerformanceMonitor.tsx');
        ...
        resolve({
          passed: issues.length === 0,
          message: issues.length > 0 ? issues.join(', ') : 'Performance optimizations are in place'
        });
      } catch (error) {
        resolve({
          passed: false,
  ...
  generateReport() {
    ...
    if (this.results.failed === 0) {
      this.log('🎉 All SEO validations passed! Your Next.js migration is SEO-ready.', 'success');
    } else {
      this.log('🔧 Please address the failed tests above to ensure optimal SEO performance.', 'warning');
    }

    this.log(`\nNEXT STEPS:`, 'header');
    this.log('1. Run "npm run build" to generate production build', 'info');
    this.log('2. Test with "npm run start" to verify SSR functionality', 'info');
    this.log('3. Use Google Search Console to test structured data', 'info');
    this.log('4. Validate social media previews with Facebook Debugger and Twitter Card Validator', 'info');
  ...
  async runAllTests() {
    this.log('Starting SEO Validation for KodeKit Next.js Migration...', 'header');
    
    await this.runTest('Server-Side Rendering Configuration', () => this.testServerSideRendering());
    await this.runTest('Metadata Generation', () => this.testMetadataGeneration());
    await this.runTest('Social Media Metadata (OG & Twitter)', () => this.testSocialMediaMetadata());
    await this.runTest('Search Engine Files (robots.txt & sitemap.xml)', () => this.testSearchEngineFiles());
    await this.runTest('Structured Data Implementation', () => this.testStructuredData());
    await this.runTest('Canonical URLs', () => this.testCanonicalUrls());
    await this.runTest('SEO-Friendly URL Structure', () => this.testUrlStructure());
