...
import { toolsData, toolCategories, ToolItem, ToolCategory } from '@/lib/data/toolsData';

export const DEFAULT_METADATA = {
  siteName: 'KodeKit',
  siteUrl: process.env.NEXT_PUBLIC_APP_URL || 'https://kodekit.in',
  twitterHandle: '@kodekit',
  defaultImage: '/social/og-image.png',
...
export function generateMetadata(config: SEOConfig): Metadata {
  ...
  const fullImage = image.startsWith('http') ? image : `${DEFAULT_METADATA.siteUrl}${image}`;
...
export function generateToolKeywords(tool: ToolItem): string[] {
  ...
  const categoryKeywords: Record<string, string[]> = {
    ...
    text: ['text formatter', 'text tool', 'string manipulation', 'text converter'],
    design: ['design tool', 'graphics tool', 'image tool', 'visual tool'],
    developer: ['code formatter', 'programming tool', 'dev tool', 'coding utility'],
    math: ['calculator', 'math tool', 'calculation', 'mathematical tool'],
    finance: ['financial calculator', 'investment tool', 'money tool', 'finance calculator'],
  ...
  return [...baseKeywords, ...specificKeywords, category?.title.toLowerCase() || '']
    .filter(Boolean)
