...
export function generateWebApplicationSchema(): WebApplicationSchema {
  ...
  return {
    ...
    description: 'A comprehensive collection of 50+ free online tools for developers, designers, and professionals. Including PDF tools, calculators, text formatters, and more.',
    url: baseUrl,
...
export function generateToolSchema(tool: ToolItem, toolName: string): ToolSchema {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://kodekit.in';
...
export function generateCategorySchema(category: ToolCategory, categoryId: string): CategorySchema {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://kodekit.in';
